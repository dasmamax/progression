...
from student_management_app.models import Matieres, CustomUser, Staffs, Courses, Subjects, Students, Attendance, AttendanceReport, LeaveReportStudent, FeedBackStudent, StudentResult, SessionYearModel
...
def add_student_save(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            first_name = form.cleaned_data['chapitre']
            last_name = form.cleaned_data['lecon']
            username = form.cleaned_data['nombre_heure']
            email = form.cleaned_data['email']
            password = form.cleaned_data['password']
            address = form.cleaned_data['address']
            session_year_id = form.cleaned_data['annee_scol_id']
            ...
            try:
                user = CustomUser.objects.create_user(
                    ...
                    # email=email,
                    nom_chapitre=first_name, last_name=last_name, user_type=2)
                user.students.address = address

                course_obj = Matieres.objects.get(id=course_id)
...
def add_subject_saves(request):
    if request.method != "POST":
        ...
        try:
            subject = Matieres(nom_matiere=subject_name)
            subject.save()
...
def prof_creer_progression(request):
    ...
    #    "staffs": staffs
    # }
    return render(request, 'student_template/prof_creer_progression.html')
